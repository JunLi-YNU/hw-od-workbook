...
/**
 * @Author: LiJun-YNU
 * @createTime: 2024年02月29日 17:02:52
 * @version: 1.0
 * @Description:
 */
...
//        secondMethod(intNumArr);
    ...
    private static void firstMethod() {
        ...
        int orderCounts = mInputScanner.nextInt();
        List<Order> orderAndSaveCounts = new ArrayList<>();
        int[] saveRoom = new int[100];
        for (int i = 0; i < orderCounts; i++) {
            String[] inputOrderString = mInputScanner.nextLine().split("=");
            orderAndSaveCounts.add(new Order(inputOrderString[0], Integer.parseInt(inputOrderString[1])));
        }
        int currentIndex = 0;
        for (Order order : orderAndSaveCounts
        ) {
            if (order.getOrderStr().equals("Re")) {
                if (getMaxRoom(saveRoom) >= order.getSaveRoomCount()) {
    ...
    public static int getMaxRoom(int[] saveRoom) {
        startIndex = 0;
        endIndex = 0;
        int maxRoom = 0;
        for (int i = 0; i < saveRoom.length; i++) {
            if (saveRoom[i] == 0) {
                startIndex = i;
                endIndex = i;
            }
            for (int j = startIndex; j < saveRoom.length; j++) {
                if (saveRoom[j] == 0) {
                    endIndex++;
                } else {
                    break;
            ...
            maxRoom = Math.max(maxRoom, endIndex - startIndex);
        }
        return maxRoom;
    }
    ...
    static class Order {
        ...
        public Order(String orderStr, int saveRoomCount) {
            this.orderStr = orderStr;
            this.saveRoomCount = saveRoomCount;
        }
        ...
        public int getSaveRoomCount() {
            return saveRoomCount;
        }

        public void setSaveRoomCount(int saveRoomCount) {
            this.saveRoomCount = saveRoomCount;
        }
    }
